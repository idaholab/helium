...
HELIUM_DIR        ?= $(CURRENT_DIR)

# moose submodule status
moose_status := $(shell git -C $(HELIUM_DIR) submodule status 2>/dev/null | grep moose | cut -c1)
ifneq (,$(findstring +,$(moose_status)))
  ifneq ($(origin HELIUM_DIR),environment)
